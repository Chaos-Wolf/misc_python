...
CC_Email = tk.Entry(fr_email_ft)
CC_Email.grid(row=0,column=6)
#bcc
tk.Label(fr_email_ft, text="BCC").grid(row=1,column=0)
BCC_Email = tk.Entry(fr_email_ft)
BCC_Email.grid(row=1,column=1)

tk.Label(fr_email_ft, text="Subject").grid(row=1,column=3)
Subject_Email = tk.Entry(fr_email_ft)
Subject_Email.grid(row=1,column=4)
#mass loader from a excel doc
Import = tk.Button(fr_email_ft, text="Import",command=lambda: importer('Email')).grid(row=1,column=2)
email_txt_edit_label = tk.Label(email, text="Message body").grid(column=0, row=2,)
#message body
Body_Email= tk.Text(email).grid(row=3,column=0)

Send_Button = tk.Button(fr_email_ft, text="Send", command=lambda: email_tools.email_send(From_Email.get(), Subject_Email.get(), email_list)).grid(row=1,column=6)
